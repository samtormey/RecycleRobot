...
octocenters = [-7 -2 6; 5 5 4; -1 4 3; 5 0 5; -3 -7 4];
ocs = octocenters;
...
for j = 1:num_octos
    verticesbottom = cat(2,cos(t)+ocs(j,1),sin(t)+ocs(j,2),ones(n,1)*(ocs(j,3) - (1/2)));
    verticestop = cat(2,cos(t)+ocs(j,1),sin(t)+ocs(j,2),ones(n,1)*(ocs(j,3) + (1/2)));
